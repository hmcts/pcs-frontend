...
test.beforeEach(async ({ page }, testInfo) => {
  ...
  if (testInfo.title.includes('@multiDefendant')) {
    ...
    process.env.CLAIMANT_NAME = claimantName;
  } else if (testInfo.title.includes('@noticeDate') || testInfo.title.includes('@noticeDetailsNo')) {
    await performAction('submitCaseAPI', { data: submitCaseApiDataWales.submitCaseNoticeDateKnown });
    claimantName = submitCaseApiDataWales.submitCaseNoticeDateKnown.claimantName;
    process.env.CLAIMANT_NAME = claimantName;
  }
...
test.describe('Respond to a claim LR - e2e Journey @nightly', async () => {
  ...
  test('RentArrears - NoticeDetails - No @LR @noticeDate', async () => {
    const pinUser = await getPinUserAt(0);
    await performAction('representationLR', {
      question: selectDefendant.whichDefendantQuestion,
    ...
    await performAction('selectWrittenTermsLR', {
      question: writtenTerms.hasYourLandlordSentYouWrittenTermsQuestion,
      radioOption: writtenTerms.yesRadioOption,
    });
    await performAction('tenancyOrContractTypeDetailsLR', {
  ...
  test('RentArrears - NoticeDetails - Yes - NoticeDate - Known @LR @noticeDetailsNo @regression', async () => {
    ...
    await performAction('enterNoticeDateKnownLR', { showNoticeDocumentLink: true });
    await performAction('rentArrearsLR', {
      option: rentArrears.yesRadioOption,
      rentArrearsTotal: submitCaseApiDataWales.submitCaseNoticeDateKnown.rentArrears_Total,
      showRentDocumentLink: true,

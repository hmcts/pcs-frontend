...
export async function defendantNameCaptureErrorValidation(): Promise<void> {
  ...
  await performValidation('errorMessage', {
    header: defendantNameCapture.thereIsAProblemErrorMessageHeader,
    message: defendantNameCapture.enterYourLastNameErrorMessage,
  });
  // Test: First name empty and last name over max length
  await performAction('inputText', defendantNameCapture.lastNameLabelText, overMaxLengthString);
  await performAction('clickButton', defendantNameCapture.saveAndContinueButton);
  await performValidation('errorMessage', {
    header: defendantNameCapture.thereIsAProblemErrorMessageHeader,
  ...
  await performAction('inputText', defendantNameCapture.firstNameLabelText, overMaxLengthString);
  await performAction('inputText', defendantNameCapture.lastNameLabelText, overMaxLengthString);
  await performAction('clickButton', defendantNameCapture.saveAndContinueButton);
  await performValidation('errorMessage', {
    header: defendantNameCapture.thereIsAProblemErrorMessageHeader,

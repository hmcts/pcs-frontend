...
- **`utils/common/pft-debug-log.ts`**: **`logPftValidationInformation`** runs after each EMV / page-navigation check. **`[PFT]`** console lines are printed **whenever** `enable_pft_debug_log === 'true'` — **pass and fail**. **Screenshots** attach to the report **only when** that check **failed**. Page navigation lines use **`page navigation validation | from page: … | action: "…" | destination page: … | url: …`** then **`expected:`** / **`actual:`** for intended vs actual landing (heading / URL).

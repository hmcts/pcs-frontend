...
export async function performActionGroup(groupName: string, ...actions: ActionStep[]): Promise<void> {
  if (!testExecutor) {
    throw new Error('Test executor not initialized. Call initializeExecutor(page) first.');
  }
...
export async function performValidationGroup(groupName: string, validations: ValidationStep[]): Promise<void> {
  if (!testExecutor) {
    throw new Error('Test executor not initialized. Call initializeExecutor(page) first.');
  }

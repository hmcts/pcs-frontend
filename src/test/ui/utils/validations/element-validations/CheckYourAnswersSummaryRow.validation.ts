...
export class CheckYourAnswersSummaryRowValidation implements IValidation {
  async validate(page: Page, validation: string, fieldName: string, data: validationRecord): Promise<void> {
    ...
    if (data.value) {
      await expect(row.locator('.govuk-summary-list__value')).toContainText(String(data.value));
    }
    if (data.linkText) {
      await expect(row.locator('.govuk-summary-list__actions a')).toContainText(String(data.linkText));
    }
  }
}

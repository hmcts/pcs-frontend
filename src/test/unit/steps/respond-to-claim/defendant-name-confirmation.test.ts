  ...
  describe('POST', () => {
    ...
    it('renders error when firstName exceeds 60 characters', async () => {
      ...
      await step.postController!.post(
        mockReq({ body: { action: 'continue', nameConfirmation: 'no', firstName: 'A'.repeat(61), lastName: 'Doe' } }),
        res,
        mockNext()
      );
    ...
    it('renders error when lastName exceeds 60 characters', async () => {
      ...
      await step.postController!.post(
        mockReq({ body: { action: 'continue', nameConfirmation: 'no', firstName: 'Jane', lastName: 'B'.repeat(61) } }),
        res,
        mockNext()
      );

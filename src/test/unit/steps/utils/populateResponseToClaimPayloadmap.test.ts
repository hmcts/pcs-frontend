...
describe('buildCcdCaseForPossessionClaimResponse', () => {
  ...
  it('applies submitted possessionClaimResponse when draft response omits cleared fields', async () => {
    (ccdCaseService.updateDraftRespondToClaim as jest.Mock).mockResolvedValue({
      id: '1773071952538472',
      data: {
        possessionClaimResponse: {
    ...
    const model = req.res?.locals?.validatedCase as InstanceType<typeof CcdCaseModel>;
    const hc = model.data.possessionClaimResponse?.defendantResponses?.householdCircumstances;
    expect(hc?.universalCredit).toBe('');
    expect(hc?.priorityDebts).toBe('NO');
  });
});

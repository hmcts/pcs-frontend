...
import { possessive } from '../../../../../main/modules/i18n/formatters/possessive';

describe('possessive formatter', () => {
  ...
  it('should register the possessive formatter', () => {
    possessive(mockI18n);

    expect(mockAdd).toHaveBeenCalledTimes(1);
    expect(mockAdd).toHaveBeenCalledWith('possessive', expect.any(Function));
  });
  ...
  describe('edge cases', () => {
    ...
    it('should handle words ending with uppercase S (case-insensitive)', () => {
      const result = formatterFn('JAMES', 'en', {});
      expect(result).toBe('JAMES’');
    });
  ...
  describe('when i18n.services.formatter is undefined', () => {
    it('should not throw when services is undefined', () => {
      ...
      expect(() => possessive(i18nWithoutServices)).not.toThrow();
    });
    ...
    it('should not throw when formatter is undefined', () => {
      ...
      expect(() => possessive(i18nWithoutFormatter)).not.toThrow();
    });
  });
});

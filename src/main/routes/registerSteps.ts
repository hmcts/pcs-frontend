...
import { caseReferenceMiddleware, ccdCaseMiddleware, oidcMiddleware } from '../middleware';
...
export default function registerSteps(app: Application): void {
  ...
  for (const [journeyName, journey] of Object.entries(journeyRegistry)) {
    ...
    for (const step of journeySteps) {
      ...
      const needsCaseReference = flowConfig.basePath?.includes('caseReference');
      const middlewares = requiresAuth
        ? [oidcMiddleware, ...(needsCaseReference ? [caseReferenceMiddleware] : []), ccdCaseMiddleware]
        : [...(needsCaseReference ? [caseReferenceMiddleware] : [])];

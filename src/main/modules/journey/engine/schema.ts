...
export const JourneySchema = z.object({
  ...
  steps: z.record(z.string(), StepSchema)
    ...
    .refine(steps => {
      ...
      error: 'Journey must have at least one form step. If it has a summary step, it must also have a confirmation step',
    })
    .refine(steps => {
      ...
      return Object.values(steps).every(step => {
        ...
        return stepIds.includes(typedStep.next.goto) && (!typedStep.next.else || stepIds.includes(typedStep.next.else));
      });

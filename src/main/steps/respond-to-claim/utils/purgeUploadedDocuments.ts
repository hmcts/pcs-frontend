...
export async function purgeUploadedDocumentsFromCdam(req: Request): Promise<void> {
  const docs: CcdCollectionItem<CcdUploadedDocument>[] | undefined =
    req.res?.locals?.validatedCase?.data?.possessionClaimResponse?.defendantResponses?.defendantDocuments;

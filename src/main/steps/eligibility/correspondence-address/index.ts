import type { Request, Response } from 'express';

import { createGetController } from '../../../app/controller/controllerFactory';
import { getFormData, setFormData } from '../../../app/controller/sessionHelper';
import { validateForm } from '../../../app/controller/validation';
import { TranslationContent, loadTranslations } from '../../../app/utils/loadTranslations';
import { getAllFormData, getBackUrl, getNextStepUrl } from '../../../app/utils/navigation';
import type { FormFieldConfig } from '../../../interfaces/formFieldConfig.interface';
import type { StepDefinition } from '../../../interfaces/stepFormData.interface';
import { SupportedLang, getLanguageFromRequest } from '../../../utils/getLanguageFromRequest';

const stepName = 'correspondence-address';

const generateContent = (lang = 'en'): TranslationContent => {
  return loadTranslations(lang, ['common', 'eligibility']);
};

const getFields = (t: TranslationContent = {}, formData?: Record<string, unknown>): FormFieldConfig[] => {
  const errors = t.errors || {};
  const isCorrespondenceAddress = formData?.isCorrespondenceAddress === 'no';

  return [
    {
      name: 'isCorrespondenceAddress',
      type: 'radio',
      required: true,
      options: [
        { value: 'yes', text: 'Yes' },
        { value: 'no', text: 'No' },
      ],
      errorMessage: errors.isCorrespondenceAddress || 'Select yes or no',
    },
    {
      name: 'addressLine1',
      type: 'text',
      required: isCorrespondenceAddress,
      errorMessage: errors.addressLine1 || 'Enter Address line 1',
    },
    {
      name: 'addressLine2',
      type: 'text',
      required: false,
    },
    {
      name: 'addressLine3',
      type: 'text',
      required: false,
    },
    {
      name: 'town',
      type: 'text',
      required: isCorrespondenceAddress,
      errorMessage: errors.town || 'Enter Town or city',
    },
    {
      name: 'county',
      type: 'text',
      required: false,
    },
    {
      name: 'postcode',
      type: 'text',
      required: isCorrespondenceAddress,
      errorMessage: errors.postcode || 'Enter a valid Postcode',
    },
    {
      name: 'country',
      type: 'text',
      required: false,
    },
  ];
};

export const step: StepDefinition = {
  url: '/steps/eligibility/correspondence-address',
  name: stepName,
  view: 'steps/eligibility/correspondenceAddress.njk',
  stepDir: __dirname,
  stepNumber: 6,
  section: 'eligibility',
  description: 'Enter correspondence address',
  prerequisites: ['page5'],
  generateContent,
  getController: (lang = 'en') => {
    const content = generateContent(lang);
    return createGetController('steps/eligibility/correspondenceAddress.njk', stepName, content, req => {
      const savedData = getFormData(req, stepName);
      return {
        ...content,
        ...savedData,
        backUrl: getBackUrl(req, stepName),
      };
    });
  },
  postController: {
    post: async (req: Request, res: Response) => {
      const lang: SupportedLang = getLanguageFromRequest(req);
      const content = generateContent(lang);
      const fields = getFields(content, req.body);
      const errors = validateForm(req, fields, content.errors);

      if (Object.keys(errors).length > 0) {
        const firstField = Object.keys(errors)[0];
        return res.status(400).render('steps/eligibility/correspondenceAddress.njk', {
          ...content,
          ...req.body,
          error: { field: firstField, text: errors[firstField] },
          errorSummaryTitle: content.errorSummaryTitle,
          backUrl: getBackUrl(req, stepName),
        });
      }

      setFormData(req, stepName, req.body);
      const allFormData = getAllFormData(req);
      const nextStepUrl = getNextStepUrl(stepName, req.body, allFormData);
      res.redirect(303, nextStepUrl);
    },
  },
};

...
export function extractDocumentId(documentUrl: string): string | undefined {
  if (!documentUrl) {
    return undefined;
  }

  // Extract UUID from URL pattern: /documents/{uuid}/binary
  const regex = /\/([0-9a-f-]{36})$/i;
...
export const documentClient = {
   ...
   * @param documentId - The DM Store document ID
  ...
  async retrieveDocument(documentId: string, accessToken: string): Promise<FileResponse> {
    try {
      const response: AxiosResponse<Buffer> = await http.get(
        `${getPcsApiUrl()}/case/document/downloadDocument/${documentId}`,
        {
          headers: {
      ...
      logger.debug(`[documentClient] Successfully retrieved document: ${fileName} (${contentType})`);

      return new FileResponse(contentType, fileName, response.data);
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : String(error);
      logger.error(`Error retrieving document ${documentId}: ${errorMessage}`);
      throw error;
    }
  },
};
